...
{
            // Opcional: Permite presionar Enter para reiniciar
            KeyboardState kState = Keyboard.GetState();
            if (kState.IsKeyDown(Keys.Enter))
            {
                // Lógica de reinicio: resetear puntuación, vidas, posición del jugador, etc.
                // currentState = GameState.Playing; 
            }
            return; // Detiene el resto del juego (movimiento, colisiones, etc.)
}
            ...
            if (currentState == GameState.GameOver)
{
            ...
            Vector2 textMeasure = ScoreFont.MeasureString(gameOverText);
            Vector2 screenCenter = new Vector2(GraphicsDevice.Viewport.Width / 2, GraphicsDevice.Viewport.Height / 2);
            Vector2 textPosition = screenCenter - (textMeasure / 2);

            // Dibuja el texto
            spriteBatch.DrawString(ScoreFont, gameOverText, textPosition, Color.Red);
        }
